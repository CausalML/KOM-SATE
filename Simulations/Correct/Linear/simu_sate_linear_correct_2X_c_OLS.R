...
for(beta in seqqbeta){  
  ...
  for(i in 1:itera){   
    ...
    res_ra_1_2 <- lm(Y ~ Z1 + Z2 + I(Z1^2) + I(Z2^2) +  Z1:Z2, data = dta[dta$Tr==1,])
    res_ra_0_2 <- lm(Y ~ Z1 + Z2 + I(Z1^2) + I(Z2^2) +  Z1:Z2, data = dta[dta$Tr==0,])
  ...
  save.image("simu_linear_correct_191018_n200_OLS.RData")
}
