...
for(beta in seqqbeta){  
  ...
  for(i in 1:itera){   
    ...
    m1 <- Match(Y = dta$Y, Tr = dta$Tr, X = fitlog$fitted, estimand = "ATE")
    m2 <- Match(Y = dta$Y, Tr = dta$Tr, X = fitlog2$fitted, estimand = "ATE")
  ...
  save.image("simu_linear_misspecified_191018_n200_matching.RData")
}
