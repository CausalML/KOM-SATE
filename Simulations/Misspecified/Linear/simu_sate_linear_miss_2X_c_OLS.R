...
for(beta in seqqbeta){  
  ...
  for(i in 1:itera){   
    ...
    if(sum(dta$Tr)<n){
        ...
        #res_na <- lm(Y ~ Tr*(Z1 + Z2), data = dta)
        
        res_ra_1_2 <- lm(Y ~ Z1 + Z2 + I(Z1^2) + I(Z2^2) +  Z1:Z2 + I(Z1^3) + I(Z2^3) + Z1:I(Z2^2) + Z2:I(Z1^2), data = dta[dta$Tr==1,])
        res_ra_0_2 <- lm(Y ~ Z1 + Z2 + I(Z1^2) + I(Z2^2) +  Z1:Z2 + I(Z1^3) + I(Z2^3) + Z1:I(Z2^2) + Z2:I(Z1^2), data = dta[dta$Tr==0,])
        pr_ra_1_2 <- predict(res_ra_1_2, newdata = dta)
        pr_ra_0_2 <- predict(res_ra_0_2, newdata = dta)
    }  
  ...
  #save.image("simu_linear_miss_191018_n200_OLS.RData")
}
